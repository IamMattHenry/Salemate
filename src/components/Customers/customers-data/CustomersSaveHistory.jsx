...
const checkAndCreateMonthlyReport = async (db) => {
  ...
  try {
    ...
    const reportData = {
      ...
      totalOrders: Array.from(customerData.values()).reduce(
        (sum, c) => sum + c.monthlyOrders,
        0
      ),
      totalSpent: Array.from(customerData.values()).reduce(
        (sum, c) => sum + c.monthlySpent,
        0
      ),
    };
...
const CustomersSaveHistory = () => {
  ...
  const updateAndDownload = async (history) => {
    try {
      ...
      const sortedCustomers = Array.from(customerData.values()).sort(
        (a, b) => b.monthlySpent - a.monthlySpent
      );
      ...
      const summaryStats = [
        ...
        `Total Customer Orders: ${sortedCustomers.reduce(
          (sum, c) => sum + c.monthlyOrders,
          0
        )}`,
        `Total Customer Sales: PHP ${sortedCustomers
      ...
      await updateDoc(historyRef, {
        ...
        totalOrders: sortedCustomers.reduce(
          (sum, c) => sum + c.monthlyOrders,
          0
        ),
    ...
    try {
      ...
      if (aprilReportSnapshot.empty && aprilData.size > 0) {
        ...
        await addDoc(collection(db, "customer_history"), {
          ...
          totalOrders: Array.from(aprilData.values()).reduce(
            (sum, c) => sum + c.monthlyOrders,
            0
          ),
          totalSpent: Array.from(aprilData.values()).reduce(
            (sum, c) => sum + c.monthlySpent,
            0
          ),
        });
        ...
        <section className="w-[94.5%] mx-auto">
          ...
          <div className="mb-6">
            ...
            <p className="text-gray-500 mt-1">
              Track customer engagement and activity patterns
            </p>
          </div>
                        ...
                        <div className="flex gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-medium text-emerald-600">
                              {Math.round(history.customerCount * 0.7)}
                            </p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-500">Inactive</p>
                            <p className="text-sm font-medium text-gray-400">
                              {Math.round(history.customerCount * 0.3)}
                            </p>
                          </div>
                        </div>
                      </td>
            ...
            {savedHistories.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="mt-4 text-sm text-gray-500">
                  No customer activity reports available
                </p>
              </div>
            )}
          </div>
