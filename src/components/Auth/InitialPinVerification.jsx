...
import { FaLock, FaCheck, FaTimes, FaEye, FaEyeSlash } from 'react-icons/fa';
import { useAuth } from '../../context/AuthContext';
import { useNavigate, useLocation } from 'react-router-dom';

const InitialPinVerification = () => {
  const { hasPin, verifyPin, createPin, userProfile, pinVerified, currentUser } = useAuth();
  ...
  const handlePinChange = async (e) => {
    ...
    if (/^\d*$/.test(value) && value.length <= 4) {
      ...
      if (value.length === 4 && !isCreatingPin) {
        ...
        setIsVerifying(true);
        try {
          const verified = await verifyPin(value);
          if (verified) {
        ...
        >
          ...
          <form onSubmit={isCreatingPin ? handleCreatePin : handleVerifyPin} className="space-y-4">
            ...
            {!success && (
              <button
                type="submit"
                className="w-full bg-amber-500 hover:bg-amber-600 text-white font-medium py-3 px-4 rounded-xl transition duration-200 mt-4 flex items-center justify-center"
